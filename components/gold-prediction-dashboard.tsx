...
export function GoldPredictionDashboard() {
  ...
  useEffect(() => {
    ...
    const next: { date: string; price: string }[] = []
    const existing = new Map(dailyEntries.map((e) => [e.date, e.price]))
    for (let d = new Date(start); d.getTime() <= end.getTime(); d.setDate(d.getDate() + 1)) {
      const key = fmt(d)
      next.push({ date: key, price: existing.get(key) ?? "" })
    }
            ...
            <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
              ...
              <CardContent className="space-y-6">
                ...
                {mode === "graph" ? (
                  <>
                    <div className="space-y-2">
                      <Label className="text-card-foreground">Daily Prices from Today to Target Date</Label>
                      <div className="grid grid-cols-1 gap-2 max-h-64 overflow-auto pr-1">
                        {dailyEntries.length === 0 ? (
                          <p className="text-xs text-muted-foreground">
                            Select a target date to generate daily inputs.
                          </p>
                        ) : (
                          dailyEntries.map((entry, idx) => (
                            <div key={entry.date} className="grid grid-cols-2 items-center gap-2">
                              ...
                              <Input
                                type="number"
                                placeholder={idx === 0 ? "Today's price" : "Enter price"}
                                value={entry.price}
                                onChange={(e) => {
                                  const next = [...dailyEntries]
                      ...
                      <p className="text-xs text-muted-foreground">
                        Fill all prices. We'll plot the curve and use the last date's price as the prediction.
                      </p>
                    </div>
                  </>
                ) : (
